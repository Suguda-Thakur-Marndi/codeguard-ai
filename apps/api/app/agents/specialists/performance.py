"""Performance Specialist Agent: audits algorithmic scaling, database query loops, and blocking calls."""
...
class PerformanceAgent(BaseSpecialistAgent):
    """Performance review specialist auditing nested loops, N+1 queries, and unbounded memory allocations."""

    def __init__(self, provider: LLMProvider):
        tier = (
            ModelTier.FAST
            if settings.MODEL_TIER_PERFORMANCE == "fast"
    ...
    async def run(
        ...
        """Run Performance Specialist Agent and return candidate findings."""
        ...
        for sym, ctx in list(context_by_symbol.items())[:8]:
            ...
            perf_context_lines.append(f"Symbol `{sym}` dependencies: {deps}")
        perf_context_str = "\n".join(perf_context_lines) if perf_context_lines else "No dependency context."
        ...
        user_prompt = prompt_tmpl.user_prompt_template.format(
            ...
            caller_context=self._truncate_context_budget(perf_context_str, 3000),
        )

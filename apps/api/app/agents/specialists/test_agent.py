"""Test & Contract Specialist Agent: verifies coverage of altered branches and interface contracts."""
...
class TestAgent(BaseSpecialistAgent):
    """Test & Contract specialist auditing whether changes are covered by tests and interface contracts respected."""
    ...
    def __init__(self, provider: LLMProvider):
        tier = (
            ModelTier.FAST
            if settings.MODEL_TIER_TEST == "fast"
    ...
    async def run(
        ...
        """Run Test Specialist Agent and return candidate findings."""
        ...
        # Extract test callers or existing tests in the repo
        test_caller_lines = []
        for sym, ctx in list(context_by_symbol.items())[:8]:
            callers = ctx.get("direct_callers", [])
            test_callers = [c for c in callers if "test" in c.lower()]
            if test_callers:
                test_caller_lines.append(f"Symbol `{sym}` is called by tests: {test_callers}")
            elif callers:
                test_caller_lines.append(f"Symbol `{sym}` is called by non-test callers: {callers[:3]} (NO DIRECT TESTS FOUND)")
        test_context_str = "\n".join(test_caller_lines) if test_caller_lines else "No test callers identified in graph."
        ...
        user_prompt = prompt_tmpl.user_prompt_template.format(
            ...
            test_and_caller_context=self._truncate_context_budget(test_context_str, 3000),
        )
